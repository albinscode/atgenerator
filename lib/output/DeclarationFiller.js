// FIXME replaceall is not really usefull... use regexes instead
    ...
    Object.keys(data).forEach(function(key) {
        ...
        if (typeof(value) === 'string') {
                ...
                if (key.indexOf('AM') === 0 || key.indexOf('PM') === 0) {
                    pattern = new RegExp(`<text:p text:style-name="\\w+"><text:user-field-get text:name="${key}">\\$\\$${key}\\$\\$</text:user-field-get></text:p>`, 'g');
                }
                else {
