        ...
        try {
            promises.push(
                ...
                self.getConnection(connectionProperties).getPage(date1.month() + 1, date1.year(), ifTimeManagement).then(function(data) {
                    ...
                    var shiftArray = 0;
                    if (datePromise.day() == 6) shiftArray = shiftArray + 2;
                    if (datePromise.add(1, 'days').day() == 0) shiftArray = shiftArray + 2;
                    log.verbose('page extractor', 'Shift array of %j %j', shiftArray, datePromise.day());
