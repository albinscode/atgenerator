...
function LinagoraConnection(connectionProperties) {
    ...
    authLinshare = async function() {
        ...
        // already authenticated
        if (this.isAutenticatedLinshare) return
        ...
        this.isAutenticatedLinshare = true
    }
     ...
     * @return a Promise
     */
    this.getPage = function (month, year, ifTimeManagement, filename) {
     ...
     * @return a Promise
     */
    this.getTimePage = function (month, year, filename) {
     ...
     * @return a Promise
     */
    this.getPlanningPage = function (month, year, filename) {
        return getBasicPage(month, year, APPLI_PLANNING_URL, false, filename)
    }

    /**
     * Creates the given folder in given workspace.
     * @Return the created folder uuid
     */
    this.createLinshareFolder = async function(folder, workspace) {

        this.authLinshare()

        workspace = '6f601740-0d99-47e4-af90-adcee6a40117'
     ...
     */
    this.uploadLinshareFile = async function(filename, workgroupUuid) {

        workgroupUuid = '6f601740-0d99-47e4-af90-adcee6a40117'
        ...
        formData.append('workGroupParentNodeUuid', '')
        ...
        try {
            ...
            if (!response.data.chunkUploadSuccess) throw new Error('Upload problem')
        }
        catch (e) {
            log.error(e)
        }
