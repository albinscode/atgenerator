...
var LinagoraConnection = require('../leech/LinagoraConnection');
...
PageExtractor.prototype.extract = async function (connectionProperties, date1, date2, ifTimeManagement, parser) {
    ...
    // to authentify
    await this.getConnection(connectionProperties)
    ...
    while (date1.isBefore(date2)) {
        promises.push(
            ...
            this.getConnection(connectionProperties)
                ...
                this.convertToObject(months, datePromiseFormat, parser.parse(data.htmlContent), shiftArray)
                return months
            })
        );
    ...
    try {
        ...
        // FIXME We have an array of the cumulated set of months
        months = months[0]
