...
function ActivityGenerator() {
    ...
    this.generateDeclarations = function(months, jsonObj, templateData) {
        try {
            ...
            var processor = new ActivityProcessorEmitter(jsonObj.activityTotal, false, jsonObj.activityProject);
            var struct = null;
            ...
            processor.on(ActivityProcessorEmitter.EVENT_MORNING_DAY_STRICT, function(value, date, numberOfDay) {
                ...
                struct['AM' + numberOfDay] = self.formatCell(value, 'AM', jsonObj.activityProject);
            });

            // each afternoon
            processor.on(ActivityProcessorEmitter.EVENT_AFTERNOON_DAY_STRICT, function(value, date, numberOfDay) {
                struct['PM' + numberOfDay] = self.formatCell(value, 'PM', jsonObj.activityProject);
