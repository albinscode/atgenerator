...
function ActivityGenerator() {
    ...
    /**
     * This is to manage weekly declarations.
     * Two types:
     * 1. followup report: all project activities, we are interested on full days coverture
     * 2. at report: one specific project activity, only days concerned by activity
     ...
     */
    this.generateWeeklyDeclarations = function(months, templateData) {
        try {
            ...
            // Report mode
                ...
                this.manageAtWeeklyReport(processor);
            }
            ...
            processor.on(ActivityProcessorEmitter.EVENT_LAST_DAY_OF_WEEK, function(weekTotal, date) {
                ...
                self.fillOneFile(promises, files, weekTotal, originalJsonObj, templateData, firstDayOfWeek);
            });
     ...
     */
    this.fillOneFile = function(promises, files, weekTotal, originalJsonObj, templateData, firstDayOfWeek) {
     ...
     */
    this.manageAtWeeklyReport = function(processor) {
        ...
        processor.on(ActivityProcessorEmitter.EVENT_AFTERNOON_DAY_STRICT, function(value, date, numberOfDay) {
            ...
            self.jsonObj.days['day' + numberOfDay] = struct;
        });
    }
     ...
     */
    this.generate = function(jsonObj, connectionProperties, ifFollowUp) {
        ...
        provider.getFromFile(jsonObj.odtTemplate).then(function(templateData) {
            ...
            extractor.extract(connectionProperties, date3, self.date2, true, parser).then(function(months) {
                self.generateWeeklyDeclarations(months, templateData);
            });
