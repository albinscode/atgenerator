    ...
    this.generateDeclarations = function(months, templateData) {
        try {
            ...
            processor.on(ActivityProcessorEmitter.EVENT_LAST_DAY_OF_WEEK, function(weekTotal, date) {
                ...
                provider.update(self.jsonObj.odtTemplate, self.jsonObj.filepath + '/' + filenamePattern, newTemplateContent, newTemplateFooter);
            });
            ...
            if (this.ifFollowUp) {
                console.log( this.jsonObj.filepath);
                var merger = require('./OdsMerger.js');
                // TODO
                merger.merge(this.jsonObj.filepath);
            }
        }
        catch (e) {
            log.error(e);
