...
JsonRules.prototype.getStartDate = function() {
    return this.getMomentValue('startDate', moment().startOf('month'));
}
...
JsonRules.prototype.getEndDate = function() {
    return this.getMomentValue('endDate', moment().endOf('month'));
}
...
JsonRules.prototype.getPlanningParser = function() {
    ...
    var parser = null;
    if (this.jsonObj.activityProject != '') {
        parser = new PlanningParser(this.jsonObj.worker, this.jsonObj.activityProject);
    } else {
        parser = new PlanningSimpleParser(this.jsonObj.worker, this.jsonObj.activityProject);
    }
    return parser;
}
