        ...
        while (date.isBefore(date2)) {
            ...
            if (timeValue !== undefined && timeValue.am && timeValue.pm && planningValue !== undefined) {
                ...
                if (timeValue && planningValue.am.indexOf(timeValue.am) == -1) {
                    log.format('diff generator', '%j, diff détéctée le matin, plannifié %j --> déclaré %j', date.format('DDMMYYYY'), planningValue.am, timeValue.am);
                    ifDiff = true;
                }
                if (timeValue && planningValue.pm.indexOf(timeValue.pm) == -1) {
                    log.format('diff generator', '%j, diff détéctée l\'après midi, plannifié %j --> déclaré %j', date.format('DDMMYYYY'), planningValue.pm, timeValue.pm);
                    ifDiff = true;
                }
            }
