var log = require('../util/LogBridge');
var fs = require('fs');
var mkdirp = require('mkdirp');

// The declaration filler
var DeclarationFiller = require('../output/DeclarationFiller.js');
var filler = new DeclarationFiller();

var PageExtractor = require('../leech/PageExtractor');

var TimeManagementParser = require('../parser/TimeManagementParser');

// To intercept on month activity data
var ActivityProcessorEmitter = require('./EventProcessor');
...
var JsonRules = require('../util/JsonRules');
...
function ActivityGenerator() {
    ...
    this.generateDeclarations = function(months, templateData) {
        try {
            ...
            if (files.length === 0) throw new Error('No file generated on this period')

            // When all file writings are ok
            return Promise.all(promises).then(function() {
                // we merge the ods files
                if (self.ifFollowUp) {
                    const filename = self.jsonObj.filepath + '/' + self.jsonObj.filenameFinalPattern;
                    return merger.merge(files, filename).then(() => log.info('generator', 'Followup file %j has been generated', filename))
                }
            });
        }
        catch (e) {
    ...
    this.manageOutOfMonth = function (date) {
        ...
        for (maxDayOfCurrentMonth; maxDayOfCurrentMonth <= maxDay; maxDayOfCurrentMonth++) {
            struct = {};
            struct['day' + maxDayOfCurrentMonth] = '';
            struct['AM' + maxDayOfCurrentMonth] = '';
            struct['PM' + maxDayOfCurrentMonth] = '';
            struct['label' + maxDayOfCurrentMonth] = '';
    ...
    this.fillOneFile = function(promises, files, total, originalJsonObj, templateData, currentDay) {
        ...
        // if followup, we don't use filenameFinalPattern, because we will merge later
        // FIXME bad design
        var filenamePattern = self.jsonObj.filenameFinalPattern && !self.ifFollowUp ? self.jsonObj.filenameFinalPattern : self.jsonObj.filenamePattern;
        ...
        filenamePattern = replaceall('$$activityProject$$', self.jsonObj.activityProject ? self.jsonObj.activityProject : '', filenamePattern);
        ...
        // we are in multi mode (applying a final pattern with user name embedded in filename)
        // we propagate new injected filename directly in json (only available when processing one filename!
        // TODO improve whole process to return list of files processed instead of global variable...
        if (self.jsonObj.filenameFinalPattern && !self.ifFollowUp) {
            self.jsonObj.filenameFinalPattern = filenamePattern
        }
    ...
    this.manageFollowupReport = function(processor) {
        ...
        processor.on(ActivityProcessorEmitter.EVENT_ONE_DAY, function(value, date, numberOfDay) {
            log.verbose("activity generator", "Processing R1, %j with value %j", date, value)
            var struct = {};
        ...
        processor.on(ActivityProcessorEmitter.EVENT_MORNING_DAY, function(value, date, numberOfDay) {
            log.verbose("activity generator", "Processing R2, %j with value %j", date, value)
            var struct = {};
            struct['day' + numberOfDay] = date.format(self.jsonObj.patternHumanDateFormat);
        ...
        processor.on(ActivityProcessorEmitter.EVENT_AFTERNOON_DAY, function(value, date, numberOfDay) {
            log.verbose("activity generator", "Processing R3, %j with value %j", date, value)
    ...
    this.manageAtReport = function(processor) {
        ...
        // each morning
        processor.on(ActivityProcessorEmitter.EVENT_MORNING_DAY_STRICT, (value, date, numberOfDay, dayOfTheMonth) => {
            struct = {};
            log.verbose("activity generator", "Processing 1, %j with value %j", date, value)
        ...
        processor.on(ActivityProcessorEmitter.EVENT_AFTERNOON_DAY_STRICT, function(value, date, numberOfDay, dayOfTheMonth) {
            log.verbose("activity generator", "Processing 2, %j with value %j", date, value)
            var day = self.getDay(numberOfDay, dayOfTheMonth);
        ...
        processor.on(ActivityProcessorEmitter.EVENT_WEEK_END, function(value, date) {
            log.verbose("activity generator", "Processing 3, %j with value %j", date, value)
            struct = {};
            var day = date.date();
            struct['day' + day] = date.format('DD');
            struct['AM' + day] = self.formatCell(value, 'AM', self.jsonObj.activityProject, date);
        ...
        // if a half day has been raised, we add specific label for monthly report
        processor.on(ActivityProcessorEmitter.EVENT_HALF_DAY, function(value, date) {
            log.verbose("activity generator", "Processing 4, %j with value %j", date, value)
            var day = date.date();
            if (self.jsonObj.days['day' + day] === undefined) {
                self.jsonObj.days['day' + day] = {}
            }
            self.jsonObj.days['day' + day]['label' + day] = '1/2 ' + self.jsonObj.companyConsultantShort;
        });

        // if a full day has been raised, we add specific label for monthly report
        processor.on(ActivityProcessorEmitter.EVENT_FULL_DAY, function(value, date) {
            log.verbose("activity generator", "Processing 5, %j with value %j", date, value)
            var day = date.date();
            if (self.jsonObj.days['day' + day] === undefined) {
                self.jsonObj.days['day' + day] = {}
            }
            self.jsonObj.days['day' + day]['label' + day] = self.jsonObj.companyConsultantShort;
        });

        // if a zero day has been raised, we empty the label for monthly report
        processor.on(ActivityProcessorEmitter.EVENT_ZERO_DAY, function(value, date) {
            log.verbose("activity generator", "Processing 6, %j with value %j", date, value)
            var day = date.date();
            if (self.jsonObj.days['day' + day] === undefined) {
                self.jsonObj.days['day' + day] = {}
            }
            self.jsonObj.days['day' + day]['label' + day] = '';
        });
    }

    this.getDay = (dayOfWeek, dayOfMonth) => {
    ...
    this.updateSpecificFields = function(date, originalJsonObj) {
        ...
        // computing number of days per week
     ...
     */
    this.generate = async function(jsonObj, connectionProperties, options) {

        // standard follow up (suivi des temps) or AT followup
        this.ifFollowUp = options.followUpReport === true

        // monthly AT report (if followup set to false), weekly otherwise
        this.ifMonthlyAtReport = options.monthlyAtReport === true

        this.parserType = typeof options.parser === 'undefined' ? 'timeParser' : options.parser
        ...
        let templateData = await provider.getFromFile(jsonObj.odtTemplate)

        let jsonRules = new JsonRules(jsonObj)
        let parser = null

        if (this.parserType === 'timeParser') {
            parser = jsonRules.getTimeParser();
        }
        else {
            parser = jsonRules.getPlanningParser();
        }

        // We extract date from time management
        // FIXME bad implementation! we have to clone?
        let months = await this.extractor.extract(connectionProperties, this.date1.clone(), this.date2, this.parserType === 'timeParser', parser)

        return this.generateDeclarations(months, templateData)
    }
