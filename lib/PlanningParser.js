...
PlanningParser.prototype.parse = function(data) {
    ...
    if (line.html() == null) {
        log.verbose('planning parser', 'There is NO data for this month related to worker %j and project code %j', this.worker, this.projectCode);
        return;
    }
    log.verbose('planning parser', 'There are data for this month related to worker %j and project code %j', this.worker, this.projectCode);
    result = [];
    var index = 0;
