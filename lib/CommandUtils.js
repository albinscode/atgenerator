...
function CommandUtils() {
     ...
     */
    this.isDisabledFeature = function(feature) {
        var found = this.disabledFeatures.filter(function (element) {
    ...
    this.buildQuestion = function(questions, question) {
        ...
        if (!this.isDisabledFeature(name)) {
            if (name === undefined) {
 ...
 * @param an array of string arguments that are disabled (to avoid asking interactive questions
 * that would be useless.
 * @return a Promise when all the command arguments and prompting is done.
 */
CommandUtils.prototype.displayPrompt = function(program, disabledFeatures) {
