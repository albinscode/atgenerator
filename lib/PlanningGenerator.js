...
function PlanningGenerator() {
    ...
    this.sumActivity = function(project, value) {
        if (this.sumObject[project] === undefined) {
            this.sumObject[project] = 0;
        }
        this.sumObject[project] = parseFloat(this.sumObject[project]) + parseFloat(value);
    };
    ...
    this.buildConsoleOutput = function (jsonObj, headers, values) {
        ...
        Object.keys(this.sumObject).forEach(function(key) {
            log.format(key + ':', self.sumObject[key]);
            sum += self.sumObject[key];
        });
        log.format('TOTAL:', sum);
    };
